...
class _TeacherCoursePlacementScreenState extends State<TeacherCoursePlacementScreen> {
  ...
  Future<Position?> _getPreciseLocation() async {
    ...
    try {
      ...
      while (attempt < maxAttempts) {
        ...
        print('📍 Tentative ${attempt + 1}/$maxAttempts - Précision: ${position.accuracy.toStringAsFixed(2)}m');
        
        // Si on a une précision ≤ 30m, on s'arrête
        if (position.accuracy <= 30) {
          print('✅ Précision excellente atteinte: ${position.accuracy.toStringAsFixed(2)}m');
      ...
      Position bestPosition = positions.reduce((a, b) => 
        a.accuracy < b.accuracy ? a : b
      );
      
      print('✅ Meilleure précision obtenue: ${bestPosition.accuracy.toStringAsFixed(2)}m');
  ...
  void _fitMapToBeacons() {
    ...
    final placedBeacons = _beacons.where((b) => b.latitude != 0 && b.longitude != 0).toList();
  ...
  Future<void> _onQrCodeDetected(BarcodeCapture capture) async {
    ...
    try {
      ...
      if (waypointId == null) {
        ...
        Beacon? beacon;
        if (qrType == 'START_COURSE' || qrType == 'START' || qrType == 'START_FINISH') {
      ...
      for (var b in _beacons) {
        try {
          ...
          print('🔍 Balise ${b.name} (id=${b.id}): waypointId=${beaconData['waypointId']}, type=${b.type}, qrType=${beaconData['type']}');
        } catch (e) {
      ...
      final beacon = _beacons.firstWhere(
        ...
        orElse: () => throw Exception('Balise non trouvée (waypointId: $waypointId)'),
      );
  ...
  Future<void> _checkCompletion() async {
    ...
    if (placedCount == totalCount && !_completionShown && totalCount > 0) {
      ...
      final success = await _courseService.updateCourseStatus(widget.course.id, 'ready');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: !_isScanning && progress < 1.0 ? FloatingActionButton.extended(
  ...
  List<Widget> _buildBeaconsList() {
    ...
    final finishBeacons = _beacons.where((b) => b.type == 'finish' || b.type == 'end').toList();
    final controlBeacons = _beacons.where((b) => b.type != 'start' && b.type != 'finish' && b.type != 'end').toList();
    ...
    return ListTile(
      ...
      title: Text(
        ...
        style: TextStyle(
          ...
          decoration: beacon.isPlaced ? TextDecoration.lineThrough : null,
        ),
      ),
      onTap: () => _showBeaconDialog(beacon),
    );
